...
💡 *The more details you give, the smarter I can be!*`;
    ...
    async analyzeDescription(description) {
        ...
        const addPropertyIfNotExists = (prop) => {
        ...
        const addEventIfNotExists = (event) => {
        ...
        const isContainerWidget = lowerDesc.includes('container') ||
        ...
        if (/\b(upload|attachment)\b/.test(lowerDesc) ||
    ...
    detectIconFile(directory) {
        ...
        try {
            ...
            const imageFiles = files.filter((file) => iconExtensions.some((ext) => file.toLowerCase().endsWith(ext)));
            ...
            const prioritized = imageFiles.sort((a, b) => {
                ...
                if (!aHasKeyword && bHasKeyword)
                    return 1;
                // Prefer SVG over PNG
                if (a.toLowerCase().endsWith('.svg') && !b.toLowerCase().endsWith('.svg'))
                    return -1;
                if (!a.toLowerCase().endsWith('.svg') && b.toLowerCase().endsWith('.svg'))
                    return 1;
                return 0;
