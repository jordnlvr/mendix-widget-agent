    ...
    generateEditorConfig(dir, config) {
        ...
        if (hasDropZones) {
            ...
            const dropZoneChildren = widgetProperties
                .map((p) => `            {
                type: "RowLayout",
                backgroundColor: values.${p.key}.widgetCount > 0 ? undefined : isDarkMode ? undefined : "#F8F8F8",
                children: [
                    {
                        type: "DropZone",
...
export function getPreview(values: ${config.name}PreviewProps, isDarkMode: boolean): StructurePreviewProps | null {
    return {
        type: "Container",
        borders: true,
        borderWidth: 1,
