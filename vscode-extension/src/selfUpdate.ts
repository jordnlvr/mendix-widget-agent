...
const CURRENT_VERSION = '2.4.2'; // Increment this with each release
  ...
  async checkForUpdates(): Promise<UpdateInfo> {
    try {
      ...
      return {
        ...
        latestVersion,
        updateAvailable,
        downloadUrl: releaseInfo.assets?.find((a: any) => a.name.endsWith('.vsix'))
...
`;
  ...
  private async buildFromSource(progressCallback?: (msg: string) => void): Promise<boolean> {
    const repoPath =
      'D:\\Users\\kelly.seale\\VSCode-Dream-Workspace\\Mendix-SDK-Workspace\\pluggable-widget-templates';
